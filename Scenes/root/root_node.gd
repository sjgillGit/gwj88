...
enum GameMode{
	MAIN_MENU, ## Should be the first scene. Keep this as #0
...
var current_mode: GameMode:
	set(v):
		_change_scene(v);                                                                                                                             print_rich("[b][color=RED]  New game mode:   [/color][/b]", GameMode.find_key(v))
		current_mode = v
